...
class RoomsController < ApplicationController
  ...
  def set_ext_params
    ...
    pass_on_join_params = lms_custom_params.select { |k, _| broker_ext_params&.[]('join')&.value?(k) }
    pass_on_create_params = lms_custom_params.select { |k, _| broker_ext_params&.[]('create')&.value?(k) }

    @room.update!(ext_params: { 'join' => pass_on_join_params, 'create' => pass_on_create_params })

    logger.debug("[Rooms\'s Controller] Set the following external params for room #{@room.id}: #{@room.ext_params.to_json}")
